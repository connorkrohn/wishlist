<script lang="ts">
	...
	const itemIndex = derived(count, ($count) =>
		$count?.findIndex((obj) => obj.id == $page.params.slug)
	);

	let itemStreamed: Awaited<typeof data.streamed.wishlist>[0], itemStreamedIndex: number;
